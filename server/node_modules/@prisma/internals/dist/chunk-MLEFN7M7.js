...
var chunk_MLEFN7M7_exports = {};
__export(chunk_MLEFN7M7_exports, {
...
module.exports = __toCommonJS(chunk_MLEFN7M7_exports);
...
var import_chunk_7YPODC6N = require("./chunk-7YPODC6N.js");
...
var import_chunk_4X6GTOPI = require("./chunk-4X6GTOPI.js");
var import_chunk_PNM45QKW = require("./chunk-PNM45QKW.js");
var import_chunk_D3RXOADJ = require("./chunk-D3RXOADJ.js");
var import_chunk_YWX5QQVJ = require("./chunk-YWX5QQVJ.js");
var import_chunk_URGOXRQA = require("./chunk-URGOXRQA.js");
var import_chunk_2IKVYFVK = require("./chunk-2IKVYFVK.js");
...
async function readSchemaFromMultiFiles(schemaPath2) {
  ...
  const config2 = await (0, import_chunk_D3RXOADJ.getConfig)({
    datamodel: files,
...
async function checkUnsupportedDataProxyMessage(command, args, implicitSchema) {
  ...
  for (const [argName, argValue] of argList) {
    ...
    if (argName.includes("schema")) {
      ...
      const config2 = await (0, import_chunk_D3RXOADJ.getConfig)({ datamodel, ignoreEnvVarErrors: true });
      const url = (0, import_chunk_D3RXOADJ.resolveUrl)((0, import_chunk_D3RXOADJ.getEffectiveUrl)(config2.datasources[0]));
...
async function getGenerators(options) {
  ...
  const config = await (0, import_chunk_D3RXOADJ.getConfig)({
    datamodel: schemas,
  ...
  const dmmf = await (0, import_chunk_YWX5QQVJ.getDMMF)({
    datamodel: schemas,
    datamodelPath: schemaPath,
    prismaPath,
    previewFeatures
...
The generator needs to either define the \`defaultOutput\` path in the manifest or you need to define \`output\` in the datamodel.prisma file.`
        ...
        const datamodel = (0, import_chunk_PNM45QKW.mergeSchemas)({ schemas });
        const options2 = {
          datamodel,
...
`
    ...
    const binaryPathsByVersion = await (0, import_chunk_7YPODC6N.getBinaryPathsByVersion)({
      neededVersions,
      binaryTarget,
      version,
      printDownloadProgress,
    ...
    for (const generator of generators) {
      if (generator.manifest && generator.manifest.requiresEngines) {
        ...
        if (engineVersion !== version && generator.options && generator.manifest.requiresEngines.includes(queryEngineType) && generatorBinaryPaths[queryEngineType] && generatorBinaryPaths[queryEngineType]?.[binaryTarget]) {
          const customDmmf = await (0, import_chunk_YWX5QQVJ.getDMMF)({
            datamodel: schemas,
            datamodelPath: schemaPath,
...
async function formatSchema({ schemas: schemas2 }, inputFormattingOptions) {
  if (process.env.FORCE_PANIC_PRISMA_SCHEMA) {
    handleFormatPanic(
      () => {
        import_chunk_2IKVYFVK.prismaSchemaWasm.debug_panic();
      },
      { schemas: schemas2 }
    );
  }
  ...
  const { formattedMultipleSchemas, lintDiagnostics } = handleFormatPanic(
    () => {
      ...
      const lintDiagnostics2 = (0, import_chunk_4X6GTOPI.lintSchema)({ schemas: formattedMultipleSchemas2 });
      return { formattedMultipleSchemas: formattedMultipleSchemas2, lintDiagnostics: lintDiagnostics2 };
    },
    { schemas: schemas2 }
  );
  const lintWarnings = (0, import_chunk_4X6GTOPI.getLintWarningsAsText)(lintDiagnostics);
...
function formatWasm(schema, documentFormattingParams) {
  const formattedSchema = import_chunk_2IKVYFVK.prismaSchemaWasm.format(schema, JSON.stringify(documentFormattingParams));
